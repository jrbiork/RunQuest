...
import type { ActivityMode, CycleDistanceAnswer, RunDistanceAnswer } from '../../src/types';

const RUN_OPTS: { value: RunDistanceAnswer; label: string; description: string; icon: string }[] = [
...
const CYCLE_OPTS: { value: CycleDistanceAnswer; label: string; description: string; icon: string }[] = [
...
export default function OnboardingDistanceScreen() {
  ...
  const [runSel, setRunSel] = useState<RunDistanceAnswer | null>(draft.current.runDistance ?? null);
  ...
  const disabled =
    mode === 'run' ? runSel === null : cycleSel === null;

  return (
    <OnboardingLayout
      step={3}
      totalSteps={5}
      title={mode === 'run' ? "What's your typical run distance?" : "What's your typical ride distance?"}
      subtitle="Distance"
      onNext={handleNext}
      nextLabel="Continue"
